...
- We need to simulate the reactions of various parties in public opinion events and information dissemination paths
...
- Views/stances (e.g., "supporters", "opponents")
...
A. **Fallback types (must include, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any natural person. When a person does not fit other more specific person types, classify here.
   - `Organization`: Fallback type for any organization. When an organization does not fit other more specific organization types, classify here.
...
- Various people appear in texts, such as "elementary school teachers", "passersby", "anonymous netizens"
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50k characters, truncate (only affects LLM input, not graph building)
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            if current_count + needed_slots > MAX_ENTITY_TYPES:
                ...
                # Remove from the end (keep more important specific types at front)

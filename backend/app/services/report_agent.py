...
4. Supports conversation with users, autonomously calling retrieval tools
"""
...
class ReportLogger:
    ...
    Generates agent_log.jsonl file in the report folder, recording each detailed action step.
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        ...
        self.log(
            ...
            stage="planning",
            details={
                "message": "Retrieving simulation context info",
                "context": context
            }
        )
    ...
    def log_section_content(
        ...
        """Record section content generation complete (content only, not the entire section)"""
        self.log(
...
This tool is used to get the complete picture of simulation results, especially suited for understanding event evolution. It will:
...
Calls the OASIS simulation environment interview API to conduct real interviews with running simulated Agents!
This is not LLM simulation, but calls the real interview interface to get original answers from simulated Agents.
...
- Need to get real answers from simulated Agents (from OASIS simulation environment)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports", with a "God's-eye view" of the simulated world -- you can observe the behavior, speech, and interactions of every Agent in the simulation.
...
- Agent speech and behavior in the simulated world are predictions of future population behavior
...
- Content should be refined, focused on core prediction findings
...
[Reminder] Report section count: minimum 2, maximum 5, content should be refined and focused on core prediction findings."""
...
and the Agents' behavior and interactions in the simulation are predictions of future population behavior.
...
2. [Must cite Agents' original speech/behavior]
   - Agents' speech and behavior are predictions of future population behavior
   - Use citation format to present these predictions in the report, for example:
...
3. [Language consistency - cited content must be translated to report language]
   ...
   - If simulation requirements and source materials are in Chinese, the report must be entirely in Chinese
   - When citing English or mixed content returned by tools, translate it into fluent Chinese before writing into the report
   - Maintain original meaning when translating, ensure natural and fluent expression
...
[Correct Example]
```
This section analyzes the event's public opinion propagation. Through in-depth analysis of simulation data, we found...
...
- Each reply calls at most one tool
...
2. Extensively cite original text to demonstrate simulation results
...
4. [Citation Format - Must Be Standalone Paragraphs]
...
- Do NOT write any headings (#, ##, ###, #### all prohibited)
...
- If information is sufficient: output section content starting with "Final Answer:" (must cite the above originals)
...
class ReportAgent:
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # In-depth interview - call real OASIS interview API for simulated Agent responses (dual-platform)
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM directly output bare JSON (no <tool_call> tags)
        # Only try when format 1 did not match, avoid false matches in body text
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section passed in with max 4000 chars
        ...
        conflict_retries = 0  # Consecutive conflict count for tool call and Final Answer appearing simultaneously
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break loop to force completion
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    # First two times: discard response, ask LLM to reply again
                    ...
                    logger.warning(
                        ...
                        "downgrading to truncated execution of first tool call"
                    )
            ...
            if has_final_answer:
                # Tool call count insufficient, reject and ask to call more tools
            ...
            # Directly use this content as final answer, no more idle looping
    ...
    def generate_report(
        ...
        """
        Generate complete report (section-by-section realtime output)
    ...
    def chat(
        ...
        In conversation, the Agent can autonomously call retrieval tools to answer questions
...
class ReportManager:
    ...
    def save_section(
        ...
        Called immediately after each section generation, enabling section-by-section output
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Since section title is added by system, content should not have any headings
    ...
    def update_progress(
        ...
        Frontend can get realtime progress by reading progress.json
        """
        cls._ensure_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-processing: clean heading issues in entire report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        3. Clean excessive blank lines and dividers
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if duplicate heading (same content heading within 5 consecutive lines)
                is_duplicate = False
                ...
                # Skip divider lines immediately after headings
                i += 1
                continue

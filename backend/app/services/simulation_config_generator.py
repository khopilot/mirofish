...
Uses LLM to automatically generate detailed simulation parameters based on simulation requirements, document content, and graph info
Fully automated, no manual parameter setup needed

Uses step-by-step generation strategy to avoid failure from generating overly long content at once:
1. Generate time config
2. Generate event config
3. Batch-generate Agent configs
...
class TimeSimulationConfig:
    ...
    off_peak_activity_multiplier: float = 0.05  # Very low early morning activity
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        # ========== Step 3-N: Batch-generate Agent configs ==========
        all_agent_configs = []
    ...
    def _build_context(
        ...
        context_parts = [
            ...
            f"\n## Entity Info ({len(entities)} total)\n{entity_summary}",
        ]
...
- 19-22 evening peak period (activity coefficient 1.5)
- After 23 activity declines (activity coefficient 0.5)
- General pattern: low activity at dawn, gradually increasing in morning, moderate during work hours, peak in evening
- **Important**: The example values below are for reference only; adjust specific time periods based on event nature and participant group characteristics
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format, time config must follow Chinese daily schedule habits."
...
For example: official statements should be published by Official/University types, news by MediaOutlet, student viewpoints by Student.
...
- **Schedule follows Chinese daily habits**: almost no activity 0-5 AM, most active 19-22 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, configs must follow Chinese daily schedule habits."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based generation of single Agent config (Chinese daily schedule)"""

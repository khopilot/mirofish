    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional, if not provided, fetched from project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True, it means it's complete
        # - running: Currently running, meaning preparation was completed long ago
        # - completed: Run complete, meaning preparation was completed long ago
        # - stopped: Stopped, meaning preparation was completed long ago
...
def prepare_simulation():
    ...
    try:
        ...
        try:
            ...
            # Save entity count to state (for frontend immediate retrieval)
                ...
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Task completed (preparation work exists)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse the reports directory, find reports matching the simulation_id.
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich information
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, fallback to recommended rounds
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g., file modification time, whether generation is in progress)
    - Can return partial information even if config hasn't been fully generated
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (complete LLM-generated configuration)
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation configuration not found, please call /prepare endpoint first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Configuration file not found, please call /prepare endpoint first"
            }), 404
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows only the current latest round's content across both platforms
...
def interview_agent():
    ...
    (entered command-waiting mode after completing simulation rounds)
...
def close_simulation_env():
    ...
    Send a close environment command to the simulation, causing it to gracefully exit command-waiting mode.

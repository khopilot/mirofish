...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wrap open() to default to UTF-8 encoding for text mode
        This fixes third-party libraries (like OASIS) that read files without specifying encoding
        """
...
# Add filter at module load time, ensuring it takes effect before camel code runs
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable OASIS library's verbose logging output
...
# Twitter available actions (excludes INTERVIEW; INTERVIEW can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excludes INTERVIEW; INTERVIEW can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        """
        # If platform specified, interview only on that platform
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Interviews needing both platforms
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows actions.jsonl to display real entity names instead of "Agent_0" style placeholders
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # If model name not in .env, use config as fallback
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Get Agent real name mapping from config (uses entity_name instead of default Agent_X)
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row in database (using rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end even with no active agents (actions_count=0)
            if action_logger:
        ...
        # Fetch actual executed actions from database and log
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation, without entering command-waiting mode'
    )

    args = parser.parse_args()

    # Create shutdown event at the start of main function to ensure entire program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: simulation does not exit after completion, waits for interview commands
    ...
    def signal_handler(signum, frame):
        ...
        # Do not sys.exit() directly; let asyncio loop exit normally and clean up resources

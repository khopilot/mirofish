        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" require active sessions and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Calculate actual height needed: rows * card height + (rows-1) * gap + small bottom padding
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Short delay (50ms) for expanding, longer (200ms) for collapsing for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Unlock after animation and check for pending state changes
          setTimeout(() => {
            isAnimating = false
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upward, requiring more scroll to trigger expansion
...
// Watch route changes, reload data when returning to homepage
...
onMounted(async () => {
  ...
  // Wait for DOM rendering before initializing observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Position from top-left, expand at bottom only during height changes */
...
.cards-container {
  ...
  /* min-height dynamically calculated by JS, adaptive to card count */
}

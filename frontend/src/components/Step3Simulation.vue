  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Old logs cleared, restarting simulation')
      }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation completed (via runner_status or platform completion)
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform completed, check if all enabled ones are done
  // Determine platform enablement by actions_count (count > 0 or was running)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // If no platform was enabled, return false

    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context to automatically organize entities and relationships from the knowledge graph, initializing simulated individuals with unique behaviors and memories based on real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, individual active periods, posting frequency, event triggers and other parameters based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, automatically generates initial activation events and trending topics to guide the simulation world initial state
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish auto-plans simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of elapsed time</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If agent count is 100: estimated time ~ {{ Math.round(customMaxRounds * 0.6) }} min</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If agent count is 100: estimated time {{ Math.round(autoGeneratedRounds * 0.6) }} min
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If running for the first time, it is strongly recommended to switch to ‘Custom Mode’ and reduce the number of simulation rounds for a quick preview and to lower the risk of errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is not less than 40 (recommended), avoid slider range issues
...
// Calculate total linked topics count for all profiles
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Initial posts: ${data.summary.initial_posts_count}posts`)
...
const loadPreparedData = async () => {
  ...
  // Fetch configuration (using real-time endpoint)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  └─ Initial posts: ${res.data.summary.initial_posts_count}posts`)
        }
        ...
        addLog('Config being generated, starting polling...')
        startConfigPolling()
      }
    }
  } catch (err) {

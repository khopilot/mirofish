...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational force toward center to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset to connection direction, calculated by distance ratio for visible curves
  ...
  // Link labels background (white bg for text clarity)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if truly started dragging (moved beyond threshold)
